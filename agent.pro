...
% takes a list of events, each of the form junk(X,Y,S), and computes the corresponding list of goals for the agent, each 
% of the form goal(X,Y,S).

% 2. incorporate_goals(Goals, Beliefs, Intentions, Intentions1)：
% has four arguments: a list of goals each of the form goal(X,Y,S), a list of beliefs (containing one term of the form 
% at(X,Y)), the current list of intentions each of the form [goal(X,Y,S), Plan], and a list to be computed which contains 
% the new goals inserted into the current list of intentions in decreasing order of value, using the distance from the 
% agent to break ties. More precisely, a new goal should be placed immediately before the first goal in the list that has 
% a lower value or which has an equal value and is further away from the agent's current position, without reordering the 
% current list of goals. Note that because of repeated perception of the same event, only new goals should be inserted 
% into the list of intentions. The plan associated with each new goal should be the empty plan (represented as the empty 
% list).

% 3. select_action(Beliefs, Intentions, Intentions1, Action)：
% takes the agent's beliefs (a singleton list containing a term for the agent's location) and the list of intentions, and 
% computes an action to be taken by the agent and the updated list of intentions. The intention selected by the agent is 
% the first on the list of intentions (if any). If the first action in this plan is applicable, the agent selects this 
% action and updates the plan to remove the selected action. If there is no associated plan (i.e. the plan is the empty 
% list) or the first action in the plan for the first intention is not applicable in the current state, the agent 
% constructs a new plan to go from its current position to the goal state and pick up the junk there (this plan will be a 
% list of move actions followed by an pick up action), selects the first action in this new plan, and updates the list of 
% intentions to incorporate the new plan (minus the selected first action). Due to the fact that there are no obstacles 
% in the world, the exact path the agent takes towards the goal does not matter, so choose any convenient way of 
% implementing this procedure.

% 4. update_beliefs(Observation, Beliefs, Beliefs1) and update_intentions(Observation, Intentions, Intentions1)：
% to compute the lists of beliefs and intentions resulting from the agent's observations. These are very simple procedures 
% for updation.
...
incorporate_goals([goal(X,Y,Value)|Goals], Beliefs, Intentions, Intentions1) :-
    % incorporate this goal into Intentions and update it.    
    incorporate_a_goal(goal(X,Y,Value), Beliefs, Intentions, TmpIntentions1),
    % continuously incorporate new goal(s).  
    incorporate_goals(Goals, Beliefs, TmpIntentions1, Intentions1),!.            
...
incorporate_a_goal(goal(X,Y,Value), [at(X_A,Y_A)], [[goal(X1,Y1,Value1),Plan]|RestOfIntentions], 
                                                [[goal(X1,Y1,Value1),Plan]|RestOfIntentions1]) :-
...
incorporate_a_goal(goal(X,Y,Value), [at(X_A,Y_A)], [[goal(X1,Y1,Value1),Plan]|RestOfIntentions], 
                                                    [[goal(X,Y,Value),[]]|RestOfIntentions1]) :-
     distance((X_A,Y_A),(X1,Y1),D_I),     
     distance((X_A,Y_A),(X,Y),D_G),
     D_G < D_I,
...
% the case that there is at least one intention with plan but the 1st action in this plan list is not 
% applicable(move distance),then construct a new plan, updating it.
select_action([at(X_A,Y_A)], [[goal(X,Y,Value),[move(X1,Y1)|RestOfPlan]]|RestOfIntentions], Intentions1, Action) :-
...
% the case that there is at least one intention with plan but the 1st action in this plan list is not 
% applicable(pickup location), then construct a new plan, updating it.
select_action([at(X_A,Y_A)], [[goal(X,Y,Value),[pickup(X1,Y1)|[]]]|RestOfIntentions], Intentions1, Action) :-
